...
mytouchend:function(){
  if (Math.abs(35.2-this.data.x)<6||(Math.abs(161.6-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(35.2-this.data.x1)<6||(Math.abs(161.6-this.data.y1))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x1:35.2,
    y1:161.6,
  ...
  if ((Math.abs(35.2-this.data.x2)<6)||((Math.abs(161.6-this.data.y2))<6)){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:35.2,
    y2:161.6,
  ...
  if (Math.abs(35.2-this.data.x3)<6||(Math.abs(161.6-this.data.y3))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x3:35.2,
    y3:161.6,
  ...
  if (Math.abs(35.2-this.data.x4)<3||(Math.abs(161.6-this.data.y4))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x4:35.2,
    y4:161.6,
  ...
  if (Math.abs(35.2-this.data.x5)<3||(Math.abs(161.6-this.data.y5))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x5:35.2,
    y5:161.6,
...
jishu:function(){
  if(this.data.x1==35.2&&this.data.y1==161.6){
...
var that=this
  ...
        success: function (res) {
    ...
    var x1=150*windowWidth/320;
     var y1=325*windowHeight/568;  
    
    var x2=4*windowWidth/320;
     var y2=407*windowHeight/568;    
     
    var x3=150*windowWidth/320;
     var y3=407*windowHeight/568;   
    
    var x4=4*windowWidth/320;
     var y4=467*windowHeight/568;    

    var x5=100*windowWidth/320;
     var y5=467*windowHeight/568;  
          that.setData({
          x:x,
          y:y,
           x1:x1,
