...
mytouchend:function(){
  if (Math.abs(179.2-this.data.x)<6||(Math.abs(161.6-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(179.2-this.data.x1)<6||(Math.abs(161.6-this.data.y1))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x1:179.2,
    y1:161.6,
  ...
  if ((Math.abs(179.2-this.data.x2)<6)||((Math.abs(161.6-this.data.y2))<6)){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:179.2,
    y2:161.6,
  ...
  if (Math.abs(179.2-this.data.x3)<6||(Math.abs(161.6-this.data.y3))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x3:179.2,
    y3:161.6,
  ...
  if (Math.abs(179.2-this.data.x4)<3||(Math.abs(161.6-this.data.y4))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x4:179.2,
    y4:161.6,
  ...
  if (Math.abs(179.2-this.data.x5)<3||(Math.abs(161.6-this.data.y5))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x5:179.2,
    y5:161.6,
  ...
  if (Math.abs(179.2-this.data.x6)<3||(Math.abs(161.6-this.data.y6))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x6:179.2,
    y6:161.6,
  ...
  if (Math.abs(179.2-this.data.x7)<3||(Math.abs(161.6-this.data.y7))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x7:179.2,
    y7:161.6,
  ...
  if (Math.abs(179.2-this.data.x8)<3||(Math.abs(161.6-this.data.y8))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x8:179.2,
    y8:161.6,
...
jishu:function(){
  if(this.data.x5==179.2&&this.data.y5==161.6){
...
move7: function (e) {
  ...
  this.setData({ x7: e.touches[0].pageX, y7: e.touches[0].pageY })//将鼠标的位置赋给图片位置数据，这样图片就可以移动了
  console.log('我正在移动')
  console.log('pageX7: ' + e.touches[0].pageX);//打印图片位置
  console.log('pageY7: ' + e.touches[0].pageY);
  //console.log(e)
},
move8: function (e) {
  
  this.setData({ x8: e.touches[0].pageX, y8: e.touches[0].pageY })//将鼠标的位置赋给图片位置数据，这样图片就可以移动了
  console.log('我正在移动')
  console.log('pageX8: ' + e.touches[0].pageX);//打印图片位置
  console.log('pageY8: ' + e.touches[0].pageY);
  //console.log(e)
},
...
var that=this
  ...
     
    ...
    var x7=123*windowWidth/320;
     var y7=480*windowHeight/568;

    var x8=236*windowWidth/320;
     var y8=480*windowHeight/568;
          that.setData({
          x:x,
          y:y,
           x1:x1,
