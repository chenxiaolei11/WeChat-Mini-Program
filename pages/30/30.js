...
mytouchend:function(){
  if (Math.abs(41.6-this.data.x1)<6||(Math.abs(162.4-this.data.y1))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(41.6-this.data.x)<6||(Math.abs(162.4-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x:41.6,
    y:162.4,
  ...
  if (Math.abs(41.6-this.data.x2)<6||(Math.abs(162.4-this.data.y2))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:41.6,
    y2:162.4,
...
jishu:function(){
  if(this.data.x1==41.6&&this.data.y1==162.4){
...
var that=this
  ...
        success: function (res) {
    ...
     var y=350*windowHeight/568;  
  
    var x1=180*windowWidth/320;
     var y1=350*windowHeight/568;  
    
    var x2=10*windowWidth/320;
     var y2=450*windowHeight/568;    
        
          that.setData({
          x:x,
          y:y,
