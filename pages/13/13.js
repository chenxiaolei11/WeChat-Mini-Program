...
mytouchend:function(){
  if (Math.abs(166.4-this.data.x)<6||(Math.abs(161.5-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(166.4-this.data.x2)<6||(Math.abs(161.5-this.data.y2))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:166.4,
    y2:161.5,
...
jishu:function(){
  if(this.data.x==166.4&this.data.y==161.5){
