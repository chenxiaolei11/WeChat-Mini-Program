...
mytouchend:function(){
  if (Math.abs(183.1-this.data.x)<6||(Math.abs(160.8-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(183.1-this.data.x1)<6||(Math.abs(160.8-this.data.y1))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x1:183.1,
    y1:160.8,
  ...
  if (Math.abs(183.1-this.data.x2)<6||(Math.abs(160.8-this.data.y2))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:183.1,
    y2:160.8,
  ...
  if (Math.abs(183.1-this.data.x3)<6||(Math.abs(160.8-this.data.y3))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x3:183.1,
    y3:160.8,
...
jishu:function(){
  if(this.data.x2==183.1&&this.data.y2==160.8){
...
var that=this
  ...
        success: function (res) {
    ...
    var x= 36*windowWidth/320;
     var y=325*windowHeight/568;  
  
    var x1=207*windowWidth/320;
     var y1=325*windowHeight/568;  
    
    var x2=36*windowWidth/320;
     var y2=407*windowHeight/568;    
     
    var x3=207*windowWidth/320;
