...
mytouchend:function(){
  if (Math.abs(40-this.data.x)<6||(Math.abs(160.8-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(40-this.data.x1)<6||(Math.abs(160.8-this.data.y1))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x1:40,
    y1:160.8,
  ...
  if ((Math.abs(40-this.data.x2)<6)||((Math.abs(160.8-this.data.y2))<6)){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:40,
    y2:160.8,
  ...
  if (Math.abs(40-this.data.x3)<6||(Math.abs(160.8-this.data.y3))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x3:40,
    y3:160.8,
  ...
  if (Math.abs(40-this.data.x4)<3||(Math.abs(160.8-this.data.y4))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x4:40,
    y4:160.8,
  ...
  if (Math.abs(40-this.data.x5)<3||(Math.abs(160.8-this.data.y5))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x5:40,
    y5:160.8,
  ...
  if (Math.abs(40-this.data.x6)<3||(Math.abs(160.8-this.data.y6))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x6:40,
    y6:160.8,
...
jishu:function(){
  if(this.data.x6==40&&this.data.y6==160.8){
...
var that=this
  ...
     
    var x3=50*windowWidth/320;
     var y3=343*windowHeight/568;   
    
    var x4=182*windowWidth/320;
