...
mytouchend:function(){
  if (Math.abs(165.5-this.data.x)<6||(Math.abs(162.5-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(165.5-this.data.x1)<6||(Math.abs(162.5-this.data.y1))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x1:165.5,
    y1:162.5,
  ...
  if (Math.abs(165.5-this.data.x2)<6||(Math.abs(162.5-this.data.y2))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:165.5,
    y2:162.5,
...
jishu:function(){
  if(this.data.x1==165.5&this.data.y1==162.5){
...
var that=this
  ...
        
          that.setData({
          x:10*windowWidth/320,
          y:400*windowHeight/568,
           x1:126*windowWidth/320,
          y1:400*windowHeight/568,
           x2:239*windowWidth/320,
          y2:400*windowHeight/568
          })
        }
      })
    },
