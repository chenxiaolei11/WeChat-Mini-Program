...
mytouchend:function(){
  if (Math.abs(164.8-this.data.x)<6||(Math.abs(184-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(164.8-this.data.x2)<6||(Math.abs(184-this.data.y2))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:164.8,
    y2:184,
...
jishu:function(){
  if(this.data.x==164.8&this.data.y==184){
