...
mytouchend:function(){
  ...
  if (Math.abs(40.7-this.data.x1)<6||(Math.abs(160.8-this.data.y1))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x1:40.7,
    y1:160.8,
  ...
  if ((Math.abs(40.7-this.data.x2)<6)||((Math.abs(160.8-this.data.y2))<6)){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:40.7,
    y2:160.8,
  ...
  if (Math.abs(40.7-this.data.x3)<6||(Math.abs(160.8-this.data.y3))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x3:40.7,
    y3:160.8,
...
jishu:function(){
  if(this.data.x3==40.7&&this.data.y3==160.8){
...
var that=this
  ...
        success: function (res) {
    ...
    var x2=20*windowWidth/320;
     var y2=407*windowHeight/568;    
     
    var x3=170*windowWidth/320;
