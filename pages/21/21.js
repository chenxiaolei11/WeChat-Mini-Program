...
mytouchend:function(){
  if (Math.abs(42.2-this.data.x)<6||(Math.abs(159.2-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(42.2-this.data.x1)<6||(Math.abs(159.2-this.data.y1))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x1:41.6,
    y1:158.4,
  ...
  if (Math.abs(40.2-this.data.x2)<6||(Math.abs(160.2-this.data.y2))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:40.2,
    y2:160.2,
...
jishu:function(){
  if(this.data.x1==41.6&this.data.y1==158.4){
...
var that=this
  ...
        success: function (res) {
    ...
    var x1=50*windowWidth/320;
     var y1=400*windowHeight/568;  
    
    var x2=160*windowWidth/320;
