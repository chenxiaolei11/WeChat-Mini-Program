...
mytouchend:function(){
  if (Math.abs(166.4-this.data.x)<6||(Math.abs(139.2-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(166.4-this.data.x1)<6||(Math.abs(139.2-this.data.y1))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x1:166.4,
    y1:139.2,
  ...
  if ((Math.abs(166.4-this.data.x2)<6)||((Math.abs(139.2-this.data.y2))<6)){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x2:166.4,
    y2:139.2,
  ...
  if (Math.abs(166.4-this.data.x3)<6||(Math.abs(139.2-this.data.y3))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x3:166.4,
    y3:139.2,
  ...
  if (Math.abs(180-this.data.x4)<3||(Math.abs(160-this.data.y4))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x4:180,
    y4:160,
  ...
  if (Math.abs(190-this.data.x5)<3||(Math.abs(172-this.data.y5))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x5:190,
    y5:172,
  ...
  if (Math.abs(166.4-this.data.x6)<3||(Math.abs(139.2-this.data.y6))<3){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x6:166.4,
    y6:139.2,
...
jishu:function(){
  if(this.data.x3==166.4&&this.data.y3==139.2){
...
var that=this
  ...
     
    ...
    var x4=100*windowWidth/320;
     var y4=407*windowHeight/568;    

    var x5=180*windowWidth/320;
     var y5=407*windowHeight/568;  

    var x6=230*windowWidth/320;
     var y6=407*windowHeight/568;
          that.setData({
          x:x,
          y:y,
           x1:x1,
