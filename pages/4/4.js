...
mytouchend:function(){
  if (Math.abs(193.6-this.data.x)<6||(Math.abs(160-this.data.y))<6){  //当小于2px的可接受阈值时，验证成功
  ...
  if (Math.abs(193.6-this.data.x1)<6||(Math.abs(160-this.data.y1))<6){  //当小于2px的可接受阈值时，验证成功
    var that=this;  //当小于2px的可接受阈值时，验证成功
    that.setData({
    x1:193.6,
    y1:160,
...
jishu:function(){
  if(this.data.x==193.6&this.data.y==160){
